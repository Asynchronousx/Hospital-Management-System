...
int send_department(int __sfd, int __choice) {
    ...
    //send the server the operation type choice we made (1,2,3)
    ...
    if(strcmp(response, "NOTFOUND") == 0) {
        puts("");
        printf("Department \033[31mnot recognized\033[0m. Make sure you entered the right number.\n");
        puts("");
        //return failure
        status = -1;
    } 
...
void scan_string(char* __string, char* __msg, int __minlenght, int __maxlenght) {
    ...
    while(!correct) {
        spaced = false;
        fgets(__string, BUFSIZE, stdin);
        __string[strlen(__string)-1] = '\0';
        size = strlen(__string);

        if(size < __minlenght || size > __maxlenght) {
            if(size < __minlenght) {
                printf("Size must be at least of %d character. Try again.\n", __minlenght);
            }
            else {
                printf("Size must be max %d character. Try again.\n", __maxlenght);
            }
            printf("%s: ", __msg);
            memset(__string, 0, size);

        } else {
            //check for spaces
            for(int i=0; i<size; i++) {
                if(__string[i] == ' ') {
                    printf("Space are not allowed. Try again.\n");
                    printf("%s: ", __msg);
                    memset(__string, 0, size);
                    spaced = true;
                    break;
